...
class ProductProduct(models.Model):
    ...
    # Mengambil value bom cost dari mrp.bom
    def _asy_bom_cost(self):
        self.ensure_one()
        bom = self.env['mrp.bom']._bom_find(product=self)
        if bom:
            return bom["asy_bom_cost"]
        return 0

    def _compute_bom_price(self, bom, boms_to_recompute=False):
        self.ensure_one()
        print("aditya compute bom price 6") #debug try
        if not bom:
            return 0
        ...
        for opt in bom.operation_ids:
            duration_expected = (
                ...
                opt.time_cycle)
            total += ((duration_expected / 60) * opt.workcenter_id.costs_hour) #Not this one
        ...
        bom_cost = self._asy_bom_cost()
        return bom.product_uom_id._compute_price(total / bom.product_qty, self.uom_id) + bom_cost
